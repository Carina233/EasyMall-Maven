...
public class OrderController extends BaseController {
	...
	public String addOrder(String receiverInfo, String cartIds, HttpSession session) {
		...
		// Orders myOrder=new
		// Orders(orderId,0.0,receiverInfo,0,timeStamp,user.getId());
	...
	public String showorder(HttpSession session, Model model) {
		User user = (User) session.getAttribute("user");
		List<OrderInfo> orderInfoList = findOrderInfoUserId(user.getId());
		model.addAttribute("orderInfos", orderInfoList);
		return "order_list";
	}

	private List<OrderInfo> findOrderInfoUserId(Integer userId) {
